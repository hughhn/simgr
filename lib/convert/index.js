...
const error = require('../../lambda/errors')
const constants = require('../constants')
const settings = require('../options')
const cache = require('../cache')
const s3 = require('../s3')

const ffmpeg = require('../../lambda/convert-ffmpeg')
...
function delegate(filename, metadata, options) {
  if (metadata.format === 'gif') {
    if (/^video\//.test(options.type)) return ffmpeg(filename, metadata, options, settings)
