...
simgr.identify = function* (metadata) {
  var id = metadata.id
  if (!id) throw new Error('metadata.id must be defined.')
...
simgr._identify = function* (metadata) {
  ...
  var stats
  try {
    stats = yield fs.stat(metadata.path)
  } catch (err) {
    // shouldn't happen
    if (err.code === 'ENOENT') throw new Error('file does not exist: ' + metadata.path)
    throw err
  }
